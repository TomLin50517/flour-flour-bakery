...
| 區塊 | 內容 | 備註 |
|---|---|---|
| Hero | 滿版影像(淡水夕陽光感)+ 品牌花 logo 置中 + slogan「為每一天,帶來美好與溫暖。」+ CTA「認識 flour flour →」 | 影像高度 85vh;文字奶油白,壓深色半透明漸層 |
| 今日出爐 | 3 款精選產品卡(圖 4:5/品名/一句描述/玫瑰金價格) | 連到 /products |
...
2. 三段式敘事(每段:小標 + 200–300 字 + 一張 3:4 圖,左右交錯):
   - 「河岸的起點」— 2024 年創立於淡水,以夕陽與河岸為日常風景的緣起
   - 「花與麵粉」— 龍眼花蜜、在地素材、自然發酵的堅持;品牌花四元素(花卉/夕陽/龍眼花/可頌弧度)的故事
   - 「每日的魔法」— 清晨開爐、限量出爐,「用魔法般的烘焙,為每一天帶來美好與溫暖」
3. 理念金句區:深可可滿版底 + 玫瑰金 serif 大字引言 + 品牌花線稿
4. 頁尾 CTA:「看看今天的麵包 →」導向 /products

### 5.3 產品介紹 `/products`

- 頁首窄幅 banner + 分類 tab(錨點切換):**經典麵包/季節限定/常溫甜點**
- 產品卡:圖 4:5(圓角 12px)→ 品名 → 一句風味描述 → 玫瑰金價格 → 「每日限量 N 個」標籤(橄欖綠 badge)
- 桌機 3 欄、平板 2 欄、手機 1 欄
- 產品資料結構(i18n JSON)含 `soldOut` 欄位:true 時卡片降飽和 + 「本日完售」badge(深可可)— v1 手動改 JSON,日後可接後台
- 虛構產品清單(融入品牌在地素材,實作時填入):
  - 經典:龍眼蜜湯種吐司 $180/魯邦鄉村 $220/海鹽可頌 $68/全麥核桃 $190
  - 季節:夕陽杏桃丹麥 $95/龍眼花蜜貝果 $85/無花果鄉村 $260/桂花栗子麵包 $110
  - 甜點:龍眼蜜瑪德蓮 $60/玫瑰荔枝達克瓦茲 $150/米粉戚風 $280/黑糖費南雪 $140
...
| 佔位圖 | 以 CSS 背景色塊 `--color-line` + 置中文字標示規格(如「Hero 16:9 淡水夕陽河岸」) |
